...
public class XylosTabControl : TabControl {
    ...
    protected override void OnPaint(PaintEventArgs e) {
        ...
        checked {
            ...
            for (int i = 0; i <= num; i++) {
                ...
                bool flag = string.IsNullOrEmpty(Convert.ToString(base.TabPages[i].Tag));
                if (flag) {
    ...
    protected override void OnSelecting(TabControlCancelEventArgs e) {
        ...
        bool flag = false;
        if (flag) {
            bool flag2 = !string.IsNullOrEmpty(Convert.ToString(e.TabPage.Tag));
            if (flag2) {
                e.Cancel = true;
            } else {
                this.OverIndex = -1;
    ...
    protected override void OnMouseMove(MouseEventArgs e) {
        ...
        checked {
            int num = base.TabPages.Count - 1;
            for (int i = 0; i <= num; i++) {
                bool flag = base.GetTabRect(i).Contains(e.Location) & base.SelectedIndex != i & string.IsNullOrEmpty(Convert.ToString(base.TabPages[i].Tag));
                if (flag) {
                    this.OverIndex = i;
                    break;
                }
